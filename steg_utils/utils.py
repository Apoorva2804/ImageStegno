...
def make_shuffled_blue(blue: np.ndarray, perm):
    ...
    """
    blocks, split_indices = split_blue_blocks(blue)
...
def unshuffle_to_visual_with_indices(blue_after_embed: np.ndarray, perm, split_indices):
    ...
    """
    blocks, _ = split_blue_blocks(blue_after_embed)
    unshuffled = [None] * 4
    for i, p in enumerate(perm):
        unshuffled[p] = blocks[i]
    return combine_blue_blocks(unshuffled, blue_after_embed.shape, split_indices)
